...
// Type: Recovered.Obfuscated.ModderTracers
...
[Mod("Modder Tracers", "Visuals", "Lines to all modders.", false, 6, ModType.Toggle, false)]
internal class ModderTracers : MonoBehaviour
{
	...
	private void Update()
	{
		...
		foreach (Player player in PhotonNetwork.PlayerListOthers)
		{
			...
			if (rig == null || rig.isOfflineVRRig || !GameNetworkUtilities.HasKnownModMarker(player))
			{
				continue;
			}
			...
			if (line == null)
			{
				line = new GameObject("ModderTracer_" + player.UserId).AddComponent<LineRenderer>();
			...
			Color color = new Color(0.5f, 0f, 0.5f, 0.6f);
