...
// Type: Recovered.Obfuscated.RopesDown
...
[Mod("Ropes Down", "World", "Moves ropes downward.", false, 27, ModType.Toggle, false)]
internal class RopesDown : MonoBehaviour
{
		...
		while (true)
		{
			...
			while (Time.time - cycleStart < 0.3f)
			{
				if (Time.time >= _lastVelocityUpdateTime + 0.02f)
				{
					...
					ApplyRopeVelocity(_currentRope, new Vector3(0f, -99f, 99f));
				}
				yield return null;
			}
