...
// Type: Recovered.Obfuscated.SpazRopes
...
[Mod("Spaz Ropes", "World", "Randomizes rope positions.", false, 29, ModType.Toggle, false)]
internal class SpazRopes : MonoBehaviour
{
		...
		while (true)
		{
			...
			while (Time.time - cycleStart < 0.3f)
			{
				if (Time.time >= _lastVelocityUpdateTime + 0.02f)
				{
					...
					Vector3 randomVelocity = new Vector3(Random.Range(-100f, 100f), Random.Range(-100f, 100f), Random.Range(-100f, 100f));
					ApplyRopeVelocity(_currentRope, randomVelocity);
				}
				yield return null;
			}
